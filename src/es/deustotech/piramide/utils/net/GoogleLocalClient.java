...
//				final URI baseURI = new URI(URI[0] + selection + URI[1] + String.valueOf(latitude) + URI[2] + String.valueOf(longitude) + URI[3] + start);
				final URI baseURI = new URI("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=43.2620151,-2.9350757&radius=1500&types=bar&key=AIzaSyC8XHVLooxK8wNN9sSqRh6F094N7_0bPoo");
...
//				}
				...
				for(int i = 0; i < results.length(); ++i){
					...
					final JSONObject geo = result.getJSONObject("geometry");
					final JSONObject loc = geo.getJSONObject("location");
					final Point point 			= new Point(result.getString("name"), 
							result.getString("vicinity"), loc.getString("lat"), 
							loc.getString("lng"));
					points.add(point);
				}
